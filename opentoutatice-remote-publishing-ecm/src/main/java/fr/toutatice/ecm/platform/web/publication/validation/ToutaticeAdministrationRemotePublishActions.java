...
public class ToutaticeAdministrationRemotePublishActions extends ToutaticeAdministrationPublishActions implements Serializable {
	...
	protected DocumentTreeNode getDocumentTreeNode(DocumentModel document) {
        ...
        if (document != null) {
            ...
            dtn = new DocumentTreeNodeImpl(document.getSessionId(), 
            		document, 
            		filter, 
            		null, 
            		sorter, 
	...
	public DocumentModelList getSelectedSections(String type) {
		...
		try {
	        ...
	        return ((ToutaticeRemoteRootSectionsFinder) getRootFinder()).getSectionRootsForWorkspace(currentDocument, true, ToutaticeRemoteRootSectionsFinder.ROOT_SECTION_TYPE.valueOf(type));
		} catch (Exception e) {
    	...
    	try {
    		...
    		if (null != parentsList && 0 < parentsList.size()) {
    			...
    			if (null != parentSectionIds && 0 < parentSectionIds.length) {
    				String[] currentDocumentSectionIds = (String[]) currentDocument.getPropertyValue(ToutaticeNuxeoStudioConst.CST_DOC_XPATH_NUXEO_SECTIONS_PROPERTY_NAME);
    				...
    				if (null != currentDocumentSectionIds && 0 < currentDocumentSectionIds.length) {
    					...
    					currentDocument.setPropertyValue(ToutaticeNuxeoStudioConst.CST_DOC_XPATH_NUXEO_SECTIONS_PROPERTY_NAME, sectionsIdMap.keySet().toArray(new String[sectionsIdMap.size()]));
    				} else {
    				...
    				facesMessages.add(StatusMessage.Severity.INFO,
    						resourcesAccessor.getMessages().get("acaren.info.admin.publication.sections.heritage"),
    						params);
    			} else {
    				// notifier l'utilisateur de la fin de l'opération
    				facesMessages.add(StatusMessage.Severity.INFO,
    						resourcesAccessor.getMessages().get("acaren.info.admin.publication.sections.heritage.none"));
    			}
    		} else {
    			// notifier l'utilisateur de la fin de l'opération
    			facesMessages.add(StatusMessage.Severity.INFO,
    					resourcesAccessor.getMessages().get("acaren.info.admin.publication.sections.heritage.nospace"));
    		}
    	} catch (Exception e) {
    ...
    public void propagateSections() {
		try {
			...
			if (null != sectionIdsList && 0 < sectionIdsList.length) {
				...
				ToutaticeNotifyEventHelper.notifyEvent(documentManager, 
						ToutaticeGlobalConst.CST_EVENT_PROPAGATE_SECTIONS, 
						currentDocument, 
						null);
				
				// notifier l'utilisateur du démarrage de l'opération
				facesMessages.add(StatusMessage.Severity.INFO,
						resourcesAccessor.getMessages().get("toutatice.info.admin.publication.sections.propagate"));
				...
				facesMessages.add(StatusMessage.Severity.WARN,
						resourcesAccessor.getMessages().get("toutatice.info.admin.publication.sections.propagate.empty"));
			}
